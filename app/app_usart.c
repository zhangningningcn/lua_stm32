...
static unsigned int UsartRxDataTick;
//static int Terminal;
...
{   
    ...
    #ifdef STM32F4XX
  /* Enable GPIO clock */
  RCC_AHB1PeriphClockCmd(USART_Lua_TX_GPIO_CLK | USART_Lua_RX_GPIO_CLK, ENABLE);
...
static void GPIO_Configuration(void)
{
  ...
  GPIO_PinAFConfig(USART_Lua_TX_GPIO_PORT, USART_Lua_TX_SOURCE, USART_Lua_TX_AF);
  GPIO_PinAFConfig(USART_Lua_RX_GPIO_PORT, USART_Lua_RX_SOURCE, USART_Lua_RX_AF);
...
int USART_Lua_Send(const char * buf,int len) {
    ...
    if(State < USB_Lua_STATE) {
        ...
        VCP_SendData(&USB_OTG_dev, (uint8_t *)buf, len);
    }
    return len;
}
int USART_Lua_SendS_from_isr(const char * buf,int len) {
    ...
    for (i = 0; i < Len; i++)
    {
        ...
        switch(State) {
            ...
            case USB_Lua_STATE_NORMAL:
                ...
                RxBuffer[RxCounter] = temp[0];
                if(temp[0] == '\n' || temp[0] == '\r') {
                    temp[0] = '\r';
                    temp[1] = '\n';
                    temp[2] = '\0';
                    VCP_SendData(&USB_OTG_dev, temp, 2);
                    USART_Lua_Send((char *)temp,2);
                    RxBuffer[RxCounter] = '\n';
                    RxState = 1;
                }
                else if(temp[0] == '\b'){
                    if(RxCounter > 0) {
                        USART_Lua_Send("\b \b",3);
                        RxCounter--;
                    }
                    break;
                }
                else {
                    USART_SendData(USART_Lua, temp[0]);
                }
				RxCounter++;
                //if(temp == '\n') {
               //     RxState = 1;
